...
def upgrade() -> None:
    op.execute("CREATE TYPE blogsessionstatus AS ENUM ('QUEUED', 'PROCESSING', 'AWAITING_OUTLINE_REVIEW', 'AWAITING_FINAL_REVIEW', 'COMPLETED', 'FAILED', 'CANCELLED')")
    op.execute("CREATE TYPE budgetentrytype AS ENUM ('GRANT', 'RESERVE', 'COMMIT', 'RELEASE', 'ADJUSTMENT')")
    ...
    op.create_table(
        ...
        sa.Column('audience', sa.String(length=255), nullable=False, server_default='general readers'),
    ...
    op.create_table(
        ...
        sa.Column('cost_usd', sa.Numeric(precision=12, scale=8), nullable=False, server_default='0'),
    ...
    op.create_table(
        ...
        sa.Column('amount_usd', sa.Numeric(precision=12, scale=8), nullable=False, server_default='0'),
