...
down_revision: str | Sequence[str] | None = "009_add_drifted_columns"
...
def upgrade() -> None:
    ...
    if not enum_exists(conn, "budget_window_type_enum"):
        ...
        conn.execute(
            sa.text("ALTER TYPE budget_window_type_enum ADD VALUE IF NOT EXISTS 'monthly'")
        )
    ...
    if not table_exists(conn, "budget_reservations"):
        op.create_table(
            ...
            sa.Column(
                "end_user_id", sa.BigInteger(), sa.ForeignKey("end_users.id"), nullable=False
            ),
            sa.Column(
        ...
        op.create_index(
            "ix_budget_reservations_tenant_end_user",
            "budget_reservations",
            ["tenant_id", "end_user_id"],
        )
        op.create_index(
            "ix_budget_reservations_blog_session", "budget_reservations", ["blog_session_id"]
        )
    ...
    if not column_exists(conn, "budget_ledger_entries", "reservation_id"):
        op.add_column(
            ...
            sa.Column(
                "reservation_id",
                sa.BigInteger(),
                sa.ForeignKey("budget_reservations.id"),
                nullable=True,
            ),
        )
    ...
    if not column_exists(conn, "budget_ledger_entries", "window_type"):
        op.add_column(
            ...
            sa.Column(
                "window_type",
                sa.Enum("daily", "weekly", "monthly", name="budget_window_type_enum"),
                nullable=False,
