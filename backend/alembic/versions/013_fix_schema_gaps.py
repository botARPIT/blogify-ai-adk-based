...
down_revision: Union[str, Sequence[str], None] = "012_drop_legacy_tables"
...
def upgrade() -> None:
    ...
    with op.get_context().autocommit_block():
        op.execute(sa.text("ALTER TYPE blog_session_status_enum ADD VALUE IF NOT EXISTS 'awaiting_budget_resolution'"))
        op.execute(sa.text("ALTER TYPE blog_session_status_enum ADD VALUE IF NOT EXISTS 'awaiting_human_review'"))
        op.execute(sa.text("ALTER TYPE blog_session_status_enum ADD VALUE IF NOT EXISTS 'awaiting_research_review'"))
