...
class TestJobPersistence:
    ...
    async def test_generate_creates_session_in_db(self):
        ...
        with patch("src.core.database.AsyncSessionFactory") as mock_factory:
            ...
            with patch("src.models.repositories.blog_session_repository.BlogSessionRepository.create") as mock_create:
    ...
    async def test_generate_creates_budget_ledger_entry(self, mock_session):
        ...
        with patch("src.models.repositories.budget_repository.BudgetRepository.write_entry") as mock_write:
    ...
    async def test_idempotency_key_prevents_duplicate_session(self, mock_session):
        ...
        with patch("src.models.repositories.blog_session_repository.BlogSessionRepository.get_by_idempotency_key") as mock_get:
...
class TestJobEnqueueDetails:
    ...
    async def test_multiple_jobs_enqueued_separately(self, mock_redis):
        ...
        with patch("src.core.redis_pool.get_redis_client", return_value=mock_redis):
            ...
            job1 = BlogJob(session_id=1, user_id=1, adk_session_id="1", topic="Topic 1", audience="a1", tone="t1", phase="start")
            job2 = BlogJob(session_id=2, user_id=1, adk_session_id="2", topic="Topic 2", audience="a2", tone="t2", phase="start")
