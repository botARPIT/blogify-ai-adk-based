...
class TestWorkerJobProcessing:
    ...
    async def test_worker_dequeues_job_from_redis(self, mock_redis):
        ...
        from src.core.task_queue import TaskQueue, BlogJob
    ...
    async def test_worker_updates_session_status_to_processing(self, mock_db_session):
        ...
        with patch("src.models.repositories.blog_session_repository.BlogSessionRepository.get_by_id") as mock_get:
            mock_get.return_value = mock_session
            
            with patch("src.models.repositories.blog_session_repository.BlogSessionRepository.update_status") as mock_update:
    ...
    async def test_worker_creates_lease_record(self, mock_db_session):
        ...
        with patch("src.models.repositories.session_lease_repository.SessionLeaseRepository.acquire_lease") as mock_acquire:
            ...
            result = await repo.acquire_lease(
                session_id=1,
                worker_id="worker-1",
                lease_seconds=300
            )
    ...
    async def test_worker_releases_lease_on_completion(self, mock_db_session):
        ...
        with patch("src.models.repositories.session_lease_repository.SessionLeaseRepository.get_current_lease") as mock_get:
            mock_get.return_value = mock_lease
            
            with patch("src.models.repositories.session_lease_repository.SessionLeaseRepository.release_lease") as mock_release:
                from src.models.repositories.session_lease_repository import SessionLeaseRepository
                repo = SessionLeaseRepository(mock_db_session)
    ...
    async def test_worker_heartbeat_extends_lease(self, mock_db_session):
        ...
        with patch("src.models.repositories.session_lease_repository.SessionLeaseRepository.heartbeat_lease") as mock_heartbeat:
...
class TestAgentRunSnapshots:
    ...
    async def test_worker_creates_agent_run_records(self, mock_db_session):
        ...
        with patch("src.models.repositories.agent_run_repository.AgentRunRepository.create") as mock_create:
    ...
    async def test_worker_creates_agent_runs_for_all_stages(self, mock_db_session):
        ...
        async def mock_create(blog_session_id, stage_name, agent_name, model_name, status, prompt_tokens=0, completion_tokens=0, total_tokens=0, cost_usd=0.0, latency_ms=None, output_snapshot=None):
            run = MagicMock()
        ...
        with patch("src.models.repositories.agent_run_repository.AgentRunRepository.create", side_effect=mock_create):
            from src.models.repositories.agent_run_repository import AgentRunRepository
            repo = AgentRunRepository(mock_db_session)
...
class TestResearchSourcesStorage:
    ...
    async def test_worker_creates_research_sources(self, mock_db_session):
        """Test worker stores research sources from Tavily."""
        sources_data = [
            {"title": "AI in Healthcare", "url": "https://example.com/1", "score": 0.9, "content": "Content 1"},
            {"title": "Machine Learning Basics", "url": "https://example.com/2", "score": 0.8, "content": "Content 2"},
        ]
        ...
        with patch("src.models.repositories.research_sources_repository.ResearchSourcesRepository.create_many", side_effect=mock_create_many):
