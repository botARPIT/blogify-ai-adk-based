...
class ValidationError(BlogifyError):
    ...
    def __init__(self, message: str, details: dict | None = None):
        super().__init__(
            message=message, error_code=ErrorCode.VALIDATION_ERROR, status_code=400, details=details
        )
...
class BudgetExceededError(BlogifyError):
    ...
    def __init__(self, message: str = "Budget limit exceeded", details: dict | None = None):
        super().__init__(
            message=message, error_code=ErrorCode.BUDGET_EXCEEDED, status_code=429, details=details
        )
...
class DatabaseError(BlogifyError):
    ...
    def __init__(self, message: str = "Database operation failed", details: dict | None = None):
        super().__init__(
            message=message, error_code=ErrorCode.DATABASE_ERROR, status_code=500, details=details
        )
...
def handle_errors(func: F) -> F:
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(
                "blogify_error", error_code=e.error_code.value, message=e.message, details=e.details
            )
...
async def generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    logger.exception(
        "unhandled_exception", request_id=request_id, error=str(exc), path=request.url.path
    )
